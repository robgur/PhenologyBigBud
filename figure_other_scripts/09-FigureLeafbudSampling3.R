...
#   - panel A label nudged left but kept on page
#   - panel A legend moved into Pacific white space
#   - panel A map returned to 100-km cells
#   - panel B species labels slightly reduced
#   - panel C relative support uses diverging colors with white at 0
# ==============================================================================
...
theme_panel <- theme_minimal(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold", size = PANEL_TITLE_SIZE, hjust = 0.03),
    plot.subtitle = element_text(size = PANEL_SUB_SIZE, margin = margin(b = 6)),
    plot.tag = element_text(face = "bold", size = PANEL_TITLE_SIZE),
    plot.tag.position = c(-0.008, 1.01),
    axis.title = element_text(size = AXIS_TITLE_SIZE),
    axis.text = element_text(size = AXIS_TEXT_SIZE),
    legend.title = element_text(size = LEGEND_TITLE_SIZE),
    legend.text = element_text(size = LEGEND_TEXT_SIZE),
    plot.margin = margin(4, 8, 4, 12)
  )
...
p_map <- ggplot() +
  ...
  labs(
    tag = "A",
    title = "Geographic sampling density",
