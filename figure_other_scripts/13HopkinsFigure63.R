...
# Compact layout with panel-local legends, titles just above map panels
# ==============================================================================
...
theme_map <- theme_void(base_size = 11) +
  theme(
    ...
    plot.title          = element_text(
      size   = 11.0, face = "bold", hjust = 0,
      vjust  = 1,
      margin = margin(t = 2, b = -21)   # <-- key change
    ),
    plot.title.position = "plot",
    plot.margin         = margin(t = 1, r = 2, b = 0, l = 2),  # <-- key change
...
make_map <- function(df, title, legend_name, lim) {
  ggplot() +
    ...
    coord_quickmap(
      ...
      expand = FALSE,
      clip   = "on"                    # keep map contained inside border
    ) +
...
# ----------------------------------------------------------------------
# Build panels — no titles in ggplot
# ----------------------------------------------------------------------

make_map <- function(df, legend_name, lim) {
  ggplot() +
    geom_raster(data = df, aes(x, y, fill = value), na.rm = FALSE) +
    geom_path(
      data      = world_df,
...
pA <- make_map(df_anom, "Hopkins deviations\n(negative = earlier)", anom_lim)
pB <- make_map(df_sens, "Advancement\nsensitivity",                  sens_lim)

fig <- pA / pB +
  plot_layout(heights = c(1, 1)) &
  theme(plot.margin = margin(2, 2, 2, 2))

# Save to a temporary file, then redraw with grid text overlaid
tmp <- tempfile(fileext = ".png")
ggsave(tmp, fig, width = 9.1, height = 3.9, dpi = 500, bg = "white")

# Re-open and annotate with grid
library(png)
img <- readPNG(tmp)

png(OUT_FIG, width = 9.1, height = 3.9, units = "in", res = 500, bg = "white")
grid.newpage()
grid.raster(img)

# Panel A label — y = 0.98 is near top of figure, adjust downward for panel B
# x = 0.01 is left edge indent
grid.text("A. Predicted Hopkins deviation",
          x = unit(0.01, "npc"), y = unit(0.98, "npc"),
          just = c("left", "top"),
          gp = gpar(fontface = "bold", fontsize = 11))

grid.text("B. Sensitivity to early forcing",
          x = unit(0.01, "npc"), y = unit(0.49, "npc"),
          just = c("left", "top"),
          gp = gpar(fontface = "bold", fontsize = 11))
